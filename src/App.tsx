...
const MacroCalculator = () => {
  ...
  const goalOptions: Record<typeof goal, string> = {
  ...
  return (
    <div className="bg-zinc-950 p-6 md:p-8 rounded-sm border border-zinc-800">
      ...
      {error && (
        <div className="bg-red-900/30 text-red-400 p-3 rounded-sm mb-4">
          {error}
        </div>
      )}
...
const PlateVisualizer = ({ counts }: { counts: Record<number, number> }) => {
      ...
      <div className="flex items-center gap-2 min-h-[52px]">
        {items.length === 0 ? (
          ...
          items.map((p, i) => (
            <div
              key={`${p}-${i}-${reverse}`}
              className={`h-12 w-8 sm:w-10 rounded-sm border flex items-end justify-center pb-1 font-black text-xs ${plateColorClass(
                p
              )}`}
              title={`${p} kg`}
            >
  ...
  return (
    <div className="bg-zinc-900 p-4 rounded-sm border border-zinc-800">
      <div className="text-xs text-zinc-500 uppercase font-bold tracking-wider mb-2">
        Visuele weergave (per kant)
      </div>
  ...
  <div className="p-5 bg-zinc-900 rounded-sm border border-zinc-800">
    ...
    {counts === null ? (
      <p className="text-red-400 font-semibold">
        Fout: Dit gewicht kan niet exact geladen worden met beschikbare
        competitieplaten in stappen van 0.5 kg.
      </p>
    ) : (
      <>
        <PlateVisualizer counts={counts} />
        <div className="mt-5 text-sm text-zinc-300">
          Gewicht per kant: {sideWeight.toFixed(2)} kg
        </div>
        {Object.keys(counts).length === 0 ? (
          <p className="text-zinc-400 mt-3">Geen platen nodig.</p>
        ) : (
...
const PlateLoader = () => {
  ...
  const results = useMemo(() => {
    ...
    const barSystemWeight = includeClips
      ? BAR_WEIGHT + COLLAR_WEIGHT_TOTAL
      : BAR_WEIGHT;
    ...
    if (platesWeight < 0) {
      return {
        counts: {},
        sideWeight: 0,
        note: `Het totale gewicht (${parsedTotal} kg) is lager dan bar${includeClips ? " + clips" : ""
          } (${barSystemWeight} kg).`,
        barSystemWeight,
      };
    }
  ...
  useEffect(() => {
    ...
    const validStep =
      ...
      Math.round(parsedTotal * 2) === parsedTotal * 2;

    if (!validStep || parsedTotal < BAR_WEIGHT) {
      setError(
        "Voer een geldig totaal gewicht in (min. 20 kg en in stappen van 0.5 kg)."
      );
    } else {
      setError("");
    }
  ...
  return (
    <div className="bg-zinc-950 p-6 md:p-8 rounded-sm border border-zinc-800">
      ...
      <p className="text-zinc-400 mb-6">
        Bereken welke competitieplaten je moet laden voor een specifiek totaal
        gewicht.
      </p>
        ...
        <div className="flex items-end">
          <label className="w-full bg-zinc-900 border border-zinc-800 p-3 rounded-sm flex items-center justify-between cursor-pointer select-none">
            ...
            <button
              ...
              className={`relative inline-flex h-7 w-12 items-center rounded-full border transition-colors ${includeClips
      ...
      {!error && results && (
        <div className="mt-6 space-y-4">
          {results.note && (
            <div className="bg-zinc-900 p-4 rounded-sm border border-zinc-800 text-zinc-400">
              {results.note}
            </div>
          )}
          ...
          <PlateResultBlock
            ...
            subtitle={`Totaal bar${includeClips ? " + clips" : ""}: ${results.barSystemWeight
              } kg`}
    ...
    <main className="min-h-screen bg-zinc-950 text-white pt-28 pb-16">
      <div className="container mx-auto px-6 max-w-6xl">
        <header className="text-center mb-10">
          <h1 className="text-4xl md:text-5xl font-black uppercase italic mb-3">
            Tools
          </h1>
          <p className="text-zinc-400 max-w-2xl mx-auto">
            Kies een tool hieronder. Wisselen doe je op deze pagina.
          </p>
        </header>
  ...
  <>
    <header className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 text-center">
        ...
        >
          Powerlifting coaching gebaseerd op topsport mentaliteit, biomechanica
          en jarenlange ervaring. Vanuit Alicante, voor atleten wereldwijd.
        </p>
        <div
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-lg mb-6 leading-relaxed">
              ...
              van lichaamsmechanica vormen de basis van mijn coaching.
            </p>
    ...
    >
      ...
      <div className="container mx-auto px-6 grid md:grid-cols-3 gap-8">
        ...
        <div className="bg-zinc-950 p-8 border border-zinc-800 hover:border-orange-500/50 transition-colors group">
          ...
          <p className="text-zinc-400 leading-relaxed">
            Voeding is brandstof. Ik focus op darmgezondheid en opneembaarheid.
            Wat je niet verteert, kun je niet gebruiken om te herstellen.
          </p>
        </div>
        ...
        <div className="bg-zinc-950 p-8 border border-zinc-800 hover:border-orange-500/50 transition-colors group">
          ...
          <p className="text-zinc-400 leading-relaxed">
            Ik ben direct. Als je techniek niet goed is, zeg ik het. Ik coach
            atleten die willen groeien.
          </p>
        </div>
      </div>
    </section>
          ...
          <div className="grid md:grid-cols-2 gap-12">
            <div>
              ...
              <p className="text-zinc-400 text-lg mb-6">
                Mijn coaching is niet voor iedereen. Ik zoek toewijding.
              </p>
            ...
            <div className="border-t md:border-t-0 md:border-l border-zinc-800 pt-8 md:pt-0 md:pl-12 flex flex-col justify-center">
              ...
              <ul className="space-y-3 text-zinc-400">
                ...
                <li className="flex items-center gap-3">
                  <X size={18} className="text-red-600 min-w-[18px]" /> Atleten
                  die niet willen communiceren
                </li>
                <li className="flex items-center gap-3">
                  <X size={18} className="text-red-600 min-w-[18px]" /> Quick fix
                  zoekers
                </li>
              </ul>
            </div>
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="md:w-1/2 bg-zinc-900 p-8 rounded-sm border-l-4 border-orange-500 shadow-lg">
          ...
          <p className="text-zinc-400 italic">
            Ik heb vaak atleten met allergieën. Als jij niet tegen lactose kan,
            of gluten, of bepaalde groenten, bouwen we daar omheen. Het doel
            blijft: maximaal resultaat zonder darmklachten.
          </p>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          <div className="bg-zinc-950 p-8 border border-zinc-800 flex flex-col relative">
            ...
            <div className="text-3xl font-black text-orange-500 mb-6">
              € Op aanvraag
              <span className="text-sm text-zinc-500 font-normal"> / maand</span>
            </div>
            <ul className="space-y-4 mb-8 flex-grow">
              <li className="flex gap-3 text-zinc-300 text-sm">
                <Check size={16} className="text-orange-500 min-w-[16px]" /> Op
                maat gemaakt schema
              </li>
              <li className="flex gap-3 text-zinc-300 text-sm">
        ...
        <div className="mt-12 text-center max-w-3xl mx-auto">
          <div className="bg-zinc-950 p-6 border border-zinc-800 inline-block text-left">
            ...
            <p className="text-zinc-400 text-sm leading-relaxed">
              Je betaalt maandelijks via automatische incasso of factuur. Geen
              verborgen kosten, wel heldere afspraken vooraf.
            </p>
          </div>
        </div>
      </div>
    ...
    <section id="contact" className="py-20 bg-orange-600 relative">
      <div className="container mx-auto px-6 text-center">
        ...
        <p className="text-orange-100 text-xl max-w-2xl mx-auto mb-10">
          Als je de inzet toont, lever ik de kennis.
        </p>
...
const App = () => {
  ...
  const [page, setPage] = useState<"home" | "tools">("home");
  ...
  return (
    <div className="bg-zinc-950 text-white min-h-screen font-sans selection:bg-orange-600 selection:text-white">
      ...
      >
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex space-x-8 font-bold text-sm tracking-wide uppercase items-center">
            <button
              onClick={goHome}
              className={`hover:text-orange-500 transition-colors ${page === "home" ? "text-orange-500" : ""
                }`}
            >
              Home
            </button>
            ...
            <button
              onClick={goTools}
              className={`hover:text-orange-500 transition-colors ${page === "tools" ? "text-orange-500" : ""
                }`}
            >
              Tools
            </button>
      ...
      {page === "home" && <HomePage scrollToSection={scrollToSection} />}
